    ...
    public static boolean isPrime(int number) {
        ...
        if (number > 1) { // первое условие проверки на простое число - оно должно быть >1
            for (int i = 1; i <= number; i++) {
                if (number % i == 0) {
                    count++; // счетчик на кол-во делителей - второе условие проверки (наличие только двух делителей)
                }
                if (count == 3) { // если делителей больше 2 смысла дальше перебирать нет
                    break;
                }
            }
        }
